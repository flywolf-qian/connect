...
import com.taiji.model.bjnodeinfo;
...
public class nodeXmlUtil1 {
    public static void createXml(String nodecode, String nodename,String version,String sn,String operator,
        ...
        if (nodecode.isEmpty() || nodename.isEmpty() || version.isEmpty() || sn.isEmpty() ||
            ...
            System.out.println("办结node节点必填数据填写异常");
        } else {
    ...
    public static  void createxml(String bjnodexml) throws DocumentException {
        Document document=DocumentHelper.parseText(bjnodexml);
        String classpath = BusinessXmlUtil.class.getResource("/").getPath();
        String filepath = classpath + File.separator + "banjie2.xml";
        xmlWrite2FileUtil.write2File(filepath, document);
    }
        public static void bjcreatexml(bjnodeinfo bjnodeinfos) throws JAXBException, DocumentException {
            String str = BeanToXml.beanToXml(bjnodeinfos, bjnodeinfo.class);
            createxml(str);
        }
